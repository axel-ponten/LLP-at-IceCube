...
def get_propagators():
    ...
    for pt in 'DeltaE', 'Brems', 'PairProd', 'NuclInt', 'WeakInt', 'Hadrons', 'EMinus', 'EPlus':
...
@icetray.traysegment
def RecreateMCTree(
    tray, name, MCTree="I3MCTree", RawMCTree="I3MCTreeRaw", RNGState="RNGState", Propagators=None, Paranoia=True
):
    """
    ...
    randomService = phys_services.I3SPRNGRandomService(seed=2, nstreams=10000, streamnum=1)
    ...
    if Paranoia:
        ...
        def assertClose(self, a, b, rtol=1e-8, atol=0, msg=None):
            ...
            if abs(a - b) > atol + rtol * abs(b):
                if atol > 0 and abs(a - b) > atol:
                    standardMsg = "%s != %s to within %s (absolute)" % (safe_repr(a), safe_repr(b), safe_repr(atol))
                else:
                    standardMsg = "%s != %s to within %s (relative)" % (safe_repr(a), safe_repr(b), safe_repr(rtol))
        ...
        class MCTreeAudit(TestCase):
            ...
            def testTotalSize(self):
                self.assertEqual(len(self.orig_tree), len(self.new_tree))
            ...
            def testParticleContent(self):
                for p1, p2 in zip(self.orig_tree, self.new_tree):
                    ...
                    self.assertClose(p1.energy, p2.energy, atol=1e-6, rtol=1e-9)
                    self.assertClose(p1.time, p2.time, atol=1, rtol=1e-9)
                    self.assertClose(p1.length, p2.length, atol=1e-6, rtol=1e-9)
                    for d in 'x', 'y', 'z':
                        self.assertClose(getattr(p1.pos, d), getattr(p2.pos, d), atol=1e-6, rtol=1e-9)
                    for d in 'zenith', 'azimuth':
                        self.assertClose(getattr(p1.dir, d), getattr(p2.dir, d), atol=1e-6, rtol=1e-9)

        tray.AddModule(
            icetray.I3TestModuleFactory(MCTreeAudit), name + 'ParanoidCheck', Streams=[icetray.I3Frame.DAQ], If=audit
        )
        tray.AddModule('Delete', name + 'DeleteParanoidCruft', Keys=[ParanoidMCTree], If=audit)
